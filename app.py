...
@app.route('/admin/violation_screenshots/<path:filename>')
def serve_violation_screenshot(filename):
    """Serve violation screenshots from the static directory"""
    return send_from_directory('static/violation_screenshots', filename)

@app.route('/admin/student_violations/<student_name>')
def student_violations(student_name):
    """Display all violation screenshots for a specific student"""
    import glob
    import os
    
    # Get the student's directory
    safe_name = "".join(c if c.isalnum() or c in (' ', '-', '_') else '_' for c in student_name)
    screenshot_dir = os.path.join('static', 'violation_screenshots', safe_name)
    
    # Get all image files in the directory
    image_files = []
    if os.path.exists(screenshot_dir):
        for ext in ['*.jpg', '*.jpeg', '*.png']:
            image_files.extend(glob.glob(os.path.join(screenshot_dir, ext)))
    
    # Convert to web paths
    violations = [os.path.relpath(f, 'static').replace('\\', '/') for f in image_files]
    
    return render_template('student_violations.html', 
                         student_name=student_name,
                         violations=violations)

@app.route('/admin/students_with_violations')
def students_with_violations():
    """List all students who have violation screenshots"""
    import os
    ...
    if os.path.exists(base_dir):
        # Get all directories (each representing a student)
        students = [d for d in os.listdir(base_dir) 
                   if os.path.isdir(os.path.join(base_dir, d))]
    
    return render_template('students_with_violations.html', students=students)
